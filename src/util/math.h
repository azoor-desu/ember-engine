...
[[nodiscard]]
consteval embF32 FastInvSqrt(embF32 value)
{
...
// Returns an approximation of 1/sqrt(value).
[[nodiscard]]
consteval embF64 FastInvSqrt(embF64 value)
{
    // Enable only on IEEE 754, else stop program
    static_assert(std::numeric_limits<embF64>::is_iec559);

    // Evil magic constant. Don't try to understand.
    // Magic value - embF32 bit-shifted by 1 as an int = approximate 1/sqrt value.
    embF64 y = std::bit_cast<embF64>(0x5F'E6'EB'50'C7'B5'37'A9 - (std::bit_cast<std::uint64_t>(value) >> 1));
...
// x to the power of y
// Only deals with positive ints.
template <typename T>
consteval T PowerIntUnsigned(T x, T y)
{
    ...
    if (y % 2 == 0)
    {
        T half_pow = PowerIntUnsigned(x, y / 2);
        return half_pow * half_pow;
    }

    return x * PowerIntUnsigned(x, y - 1);
}
